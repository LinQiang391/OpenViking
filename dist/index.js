...
function parseImportKind(value) {
    if (!value) {
        return "resource";
    }
...
export function parseOvImportCommandArgs(args) {
...
const contextEnginePlugin = {
    ...
    register(api) {
        ...
        const hasExplicitConfig = Object.keys(rawCfg).length > 0;
        const needsSetup = !hasExplicitConfig &&
            !cfg.apiKey;
        if (needsSetup) {
            api.logger.warn("openviking: no configuration detected (no apiKey, no explicit config). " +
            ...
            api.registerService({
                id: "openviking",
                start: () => {
                    api.logger.info("openviking: running in setup-only mode. " +
        ...
        const executeImport = async (input, agentId) => {
        ...
        const formatSearchText = (query, uri, result) => {
            if ((result.total ?? 0) <= 0) {
        ...
        const searchOpenViking = async (input, agentId) => {
            ...
            return {
                content: [{ type: "text", text: formatSearchText(query, input.uri, result) }],
                details: {
                    action: "searched",
                    query,
                    uri: input.uri,
        ...
        api.registerTool((ctx) => ({
            ...
            description: "Import an OpenViking resource or skill only when the user explicitly asks to import, add, or index one. " +
