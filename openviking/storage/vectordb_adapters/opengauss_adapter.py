...
class OpenGaussCollection(ICollection):
    ...
    def _save_index_meta(self, index_name: str, meta: Dict[str, Any]):
        ...
        # Use MERGE for OpenGauss compatibility (no ON CONFLICT support)
        self._execute(
            f"""
            MERGE INTO "{idx_table}" AS t
            USING (SELECT %s AS index_name, %s AS meta_json) AS s
            ON t.index_name = s.index_name
            WHEN MATCHED THEN UPDATE SET meta_json = s.meta_json
            WHEN NOT MATCHED THEN INSERT (index_name, meta_json) VALUES (s.index_name, s.meta_json)
            """,
    ...
    def upsert_data(self, data_list: List[Dict[str, Any]], ttl: int = 0):
        ...
        for record in data_list:
            ...
            # Build update assignments for MERGE (use s. prefix for source values)
            update_set = ", ".join(
                f'"{c}" = s."{c}"' for c in col_names if c != "id"
            )
            # Build source column list for MERGE with proper casting
            source_cols = ", ".join(
                f'{p} AS "{c}"' for c, p in zip(col_names, col_placeholders)
            )
            insert_cols = ", ".join(f's."{c}"' for c in col_names)

            # Use MERGE for OpenGauss compatibility (no ON CONFLICT support)
            sql = f"""
                MERGE INTO "{self._name}" AS t
                USING (SELECT {source_cols}) AS s
                ON t.id = s.id
                WHEN MATCHED THEN UPDATE SET {update_set}
                WHEN NOT MATCHED THEN INSERT ({cols_sql}) VALUES ({insert_cols})
            """
            try:
...
def _save_collection_meta(conn, name: str, meta: Dict[str, Any], distributed: bool = False):
    ...
    try:
        ...
        cur.execute(
            f"""
            MERGE INTO "{_META_TABLE}" AS t
            USING (SELECT %s AS table_name, %s AS meta_json) AS s
            ON t.table_name = s.table_name
            WHEN MATCHED THEN UPDATE SET meta_json = s.meta_json
            WHEN NOT MATCHED THEN INSERT (table_name, meta_json) VALUES (s.table_name, s.meta_json)
            """,
