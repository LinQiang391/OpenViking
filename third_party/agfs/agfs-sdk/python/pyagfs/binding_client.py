...
def _find_library() -> str:
    ...
    search_paths = [
        ...
        Path(os.environ.get("AGFS_LIB_PATH", "")) / lib_name if os.environ.get("AGFS_LIB_PATH") else None,
        Path("/tmp") / lib_name,
    ]
...
class BindingLib:
    ...
    def _setup_functions(self):
        ...
        self.lib.AGFS_Read.argtypes = [
            ctypes.c_int64, ctypes.c_char_p, ctypes.c_int64, ctypes.c_int64,
        ...
        self.lib.AGFS_OpenHandle.argtypes = [
            ctypes.c_int64, ctypes.c_char_p, ctypes.c_int, ctypes.c_uint, ctypes.c_int
        ]
        ...
        self.lib.AGFS_HandleWrite.argtypes = [
            ctypes.c_int64, ctypes.c_void_p, ctypes.c_int64, ctypes.c_int64, ctypes.c_int
        ]
        self.lib.AGFS_HandleWrite.restype = ctypes.c_char_p
        
        self.lib.AGFS_HandleSeek.argtypes = [
...
class AGFSBindingClient:
    ...
    def ls(self, path: str = "/") -> List[Dict[str, Any]]:
        ...
        result = self._lib.lib.AGFS_Ls(
            self._client_id, path.encode('utf-8')
        )
    ...
    def create(self, path: str) -> Dict[str, Any]:
        """Create a new file."""
        result = self._lib.lib.AGFS_Create(
            self._client_id, path.encode('utf-8')
        )
        return self._parse_response(result)
    ...
    def rm(self, path: str, recursive: bool = False) -> Dict[str, Any]:
        ...
        result = self._lib.lib.AGFS_Rm(
            self._client_id, path.encode('utf-8'), 1 if recursive else 0
        )
        return self._parse_response(result)
    
    def stat(self, path: str) -> Dict[str, Any]:
        """Get file/directory information."""
        result = self._lib.lib.AGFS_Stat(
            self._client_id, path.encode('utf-8')
        )
        return self._parse_response(result)
    ...
    def touch(self, path: str) -> Dict[str, Any]:
        """Touch a file."""
        result = self._lib.lib.AGFS_Touch(
            self._client_id, path.encode('utf-8')
        )
        return self._parse_response(result)
    ...
    def unmount(self, path: str) -> Dict[str, Any]:
        """Unmount a plugin."""
        result = self._lib.lib.AGFS_Unmount(
            self._client_id, path.encode('utf-8')
        )
        return self._parse_response(result)
    ...
    def load_plugin(self, library_path: str) -> Dict[str, Any]:
        ...
        result = self._lib.lib.AGFS_LoadPlugin(
            self._client_id, library_path.encode('utf-8')
        )
        return self._parse_response(result)
    
    def unload_plugin(self, library_path: str) -> Dict[str, Any]:
        """Unload an external plugin."""
        result = self._lib.lib.AGFS_UnloadPlugin(
            self._client_id, library_path.encode('utf-8')
        )
        return self._parse_response(result)
    ...
    def open_handle(self, path: str, flags: int = 0, mode: int = 0o644, 
                    lease: int = 60) -> 'FileHandle':
    ...
    def handle_read(self, handle_id: int, size: int = -1, 
    ...
    def handle_write(self, handle_id: int, data: bytes, 
