    ...
    async def _tick(self) -> None:
        ...
        for workspace_path, session_key_list in workspaces.items():
            ...
            if self.on_heartbeat:
                try:
                    logger.debug(f"Heartbeat: calling on_heartbeat for {workspace_path} with prompt: {HEARTBEAT_PROMPT[:100]}...")
                    for session_key in session_key_list:
                        response = await self.on_heartbeat(HEARTBEAT_PROMPT, session_key)
                        logger.debug(f"Heartbeat: received response from agent: {response[:200]}...")
                        ...
                        if response_clean == heartbeat_ok_clean or response_clean.startswith(heartbeat_ok_clean):
