...
from opencode_utils import check_serve_status, execute_cmd, read_new_messages, read_status, write_status, list_project
...
try:
    ...
    for session in sessions:
        ...
        v_session = ViewSession(
            ...
            last_modify_time=time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(session.get('updated')/1000)),
            create_time=time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(session.get('created')/1000)),
            directory=session.get('directory'),
            status='',
            new_messages=''
        )
        status, new_messages_list, next_ts = read_new_messages(client, session_id=v_session.session_id, last_ts=session_notify_status.get(v_session.session_id, 0))
