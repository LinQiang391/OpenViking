...
| Node.js | `>= 22` |
| OpenClaw | `>= 2026.3.7` |
| OpenViking server | Reachable HTTP service, for example `http://127.0.0.1:1933` |
...
`ov-install` is not the recommended path for normal users. It exists for developers who need to install or test plugin files from arbitrary Git branches before publishing an npm package.

Use it when you are developing the plugin itself, testing a branch, or migrating a local development environment.
...
For user-facing installs, prefer `openclaw plugins install @openclaw/openviking`.
