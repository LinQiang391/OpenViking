...
function readCompatRangeFromManifest() {
    try {
        const thisDir = path.dirname(fileURLToPath(import.meta.url));
        const manifestPath = path.join(thisDir, "..", "install-manifest.json");
...
async function askAgentPrefix(zh, q, defaultValue) {
    while (true) {
        const value = (await q(tr(zh, "Agent Prefix (optional)", "Agent Prefix（可选）"), defaultValue)).trim();
        if (isValidAgentPrefixInput(value)) {
            return value;
        }
        console.log(`  ✗ ${tr(zh, "Agent Prefix may only contain letters, digits, underscores, and hyphens, or be empty.", "Agent Prefix 只能包含字母、数字、下划线和连字符，或留空。")}`);
    }
}
...
function capture(cmd, args, opts) {
    return new Promise((resolve) => {
        ...
        child.on("close", (code) => { resolve({ code: code ?? -1, out: out.trim(), err: errOut.trim() }); });
    });
}
async function resolveAbsoluteCommand(cmd) {
    if (cmd.startsWith("/") || (IS_WIN && /^[A-Za-z]:[/\\]/.test(cmd)))
        return cmd;
    if (IS_WIN) {
...
export function registerSetupCli(api) {
    ...
    api.registerCli(({ program }) => {
        ...
        ovCmd
            ...
            const { reconfigure, zh: zhOpt, baseUrl, apiKey, agentPrefix, accountId, userId, allowOffline, forceSlot, json: jsonOpt, } = options;
            ...
            try {
                if (existing && !reconfigure) {
                    ...
                    if (existing.agent_prefix)
                        console.log(`  agent_prefix: ${existing.agent_prefix}`);
                    console.log("");
...
async function setupNonInteractive(configPath, params) {
    try {
        ...
        if (agentPrefix)
            pluginCfg.agent_prefix = agentPrefix;
        if (accountId)
...
function printSetupResult(zh, result) {
    ...
    if (result.success) {
        ...
        if (result.config) {
            ...
            if (result.config.agent_prefix)
                console.log(`  agent_prefix: ${result.config.agent_prefix}`);
...
async function setupRemote(zh, configPath, existing, q) {
    ...
    if (agentPrefix)
        console.log(`  agent_prefix: ${agentPrefix}`);
    if (accountId)
