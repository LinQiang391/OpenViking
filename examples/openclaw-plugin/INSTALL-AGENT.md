...
| Upgrade plugin to latest | `npm install -g openclaw-openviking-setup-helper@latest && ov-install` |
| Install or upgrade a specific release | `npm install -g openclaw-openviking-setup-helper@latest && ov-install --plugin-version=v0.2.9` |
...
## Developer Path: ov-install

`ov-install` is for plugin developers, not normal users. Use it when testing a Git branch, local development build, or helper-managed migration before publishing an npm package.
...
| Intent | Command |
| --- | --- |
| Install from a development ref | `ov-install --plugin-version=<REF>` |
| Non-interactive developer install | `ov-install --base-url <URL> --api-key <KEY>` |
...
For user installs, do not choose `ov-install`. Use `openclaw plugins install @openclaw/openviking`.
