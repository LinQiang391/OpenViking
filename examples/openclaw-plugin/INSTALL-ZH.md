...
| Node.js | `>= 22` |
| OpenClaw | `>= 2026.3.7` |
| OpenViking 服务 | 可通过 HTTP 访问，例如 `http://127.0.0.1:1933` |
...
`ov-install` 不是普通用户推荐安装方式。它面向插件开发者，用于在 npm 包发布前从任意 Git 分支、标签或开发版本安装插件文件。

适用场景：开发本插件、测试分支、迁移本地开发环境。
...
面向用户的安装，请优先使用 `openclaw plugins install @openclaw/openviking`。
