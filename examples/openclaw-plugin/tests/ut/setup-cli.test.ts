...
describe("checkVersionCompatibility", () => {
  ...
  it('returns "compatible" when no min version is set', () => {
    expect(checkVersionCompatibility("0.1.0")).toBe("compatible");
  });
});
...
describe("ensureInstallRecord", () => {
  it("creates installs and allow entries when missing", () => {
    const plugins: Record<string, unknown> = {};
    ensureInstallRecord(plugins);
    expect(plugins.installs).toEqual({ openviking: { npm: "@openclaw/openviking" } });
    expect(plugins.allow).toEqual(["openviking"]);
  });

  it("preserves existing installs and allow entries", () => {
    const plugins: Record<string, unknown> = {
      installs: { mem0: { npm: "@mem0/openclaw-mem0" } },
      allow: ["mem0"],
    };
    ensureInstallRecord(plugins);
    expect((plugins.installs as Record<string, unknown>).mem0).toEqual({
      npm: "@mem0/openclaw-mem0",
    });
    expect((plugins.installs as Record<string, unknown>).openviking).toEqual({
      npm: "@openclaw/openviking",
    });
    expect(plugins.allow).toEqual(["mem0", "openviking"]);
  });

  it("does not duplicate when already registered", () => {
    const plugins: Record<string, unknown> = {
