...
async function resolvePythonCmd(): Promise<string> {
  ...
  const envFile = IS_WIN
    ? path.join(HOME, ".openclaw", "openviking.env.bat")
    : path.join(HOME, ".openclaw", "openviking.env");
...
async function checkOpenVikingInstalled(): Promise<{ ok: boolean; version: string; pythonPath: string }> {
  ...
  for (const candidate of candidates) {
    ...
    if (!resolved || resolved === pythonCmd) continue;
...
function writeOpenvikingEnv(pythonPath: string): void {
  ...
  if (IS_WIN) {
    const batPath = path.join(openclawDir, "openviking.env.bat");
    fs.writeFileSync(batPath, `@echo off\r\nset "OPENVIKING_PYTHON=${pythonPath}"\r\n`, "utf-8");
    const ps1Path = path.join(openclawDir, "openviking.env.ps1");
    fs.writeFileSync(ps1Path, `$env:OPENVIKING_PYTHON = "${pythonPath.replace(/\\/g, "\\\\")}"\n`, "utf-8");
  } else {
    const envPath = path.join(openclawDir, "openviking.env");
    fs.writeFileSync(envPath, `export OPENVIKING_PYTHON='${pythonPath}'\n`, "utf-8");
  }
}
